...
<AddCustomerModal v-if="show_AddModal" @close-modal="show_AddModal = false" :refreshData="refreshData" :item_data="item_data" :showNotification="showNotification"/>
...
getAll(type,name){
  ...
  if(name){
    ...
    axios.get(process.env.VUE_APP_BASE_URL + '/user/get_all_pending_filter/' + this.page + '/' + this.page_limit + '/' + 'customer' + '/' + type + '/' + name)
    .then((res)=>{
    ...
    axios.get(process.env.VUE_APP_BASE_URL + '/user/get_all_pending/' + this.page + '/' + this.page_limit + '/' + 'customer')
    .then((res)=>{
