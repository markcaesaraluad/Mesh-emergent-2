<template>
    <div>
      ...
      <div class="sm:flex items-center sm:space-x-4 space-y-4 sm:space-y-0">
        ...
        <input v-model="search_name" type="text" class="flex-auto border rounded-md px-8 text-sm" placeholder="Search employee here..." @keyup.enter="getAll('search',search_name)">
                          ...
                          <tr v-for="i in alldata" v-bind:key="i.id" class="hover:bg-gray-200">
                            ...
                            <td class="py-4 px-4 text-center w-16">
                              <div class="flex items-center gap-2">
                                ...
                                <!-- <img src="../assets/icon_list.png" @click="showAddModal(i.id, 'edit')" class="h-4 w-auto cursor-pointer mx-auto"> -->
                              </div>
  ...
  getAll(type,name){
    ...
    if(name){
      ...
      axios.get(process.env.VUE_APP_BASE_URL + '/user/get_all/' + this.page + '/' + this.page_limit + '/' + 1 + '/' + type + '/' + name)
      .then((res)=>{
