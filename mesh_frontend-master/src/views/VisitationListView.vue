<template>
    <div>
      ...
      <div class="sm:flex items-center sm:space-x-4 space-y-4 sm:space-y-0">
        ...
        <input v-model="selected_date" type="date" class="w-auto rounded-md flex-none" @change="getAll(null,null)">
  ...
  <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
    ...
    <table class="min-w-full divide-y divide-gray-300">
                          ...
                          <tr class="text-white">
                              ...
                              <th scope="col" class="py-4 px-4 text-gray-500 font-bold text-center text-sm w-1/4">Package Availed</th>
                          </tr>
                          </thead>
                          <tbody class="divide-y divide-gray-200 bg-white">
                          <tr v-for="i in alldata" v-bind:key="i.id" class="hover:bg-gray-200">
  
                            <td class="py-4 px-4 text-sm text-gray-500 sm:pl-6 w-auto">{{formatUserName(i.user_id)}}</td>
                            <td class="py-4 px-4 text-sm text-gray-500 sm:pl-6 w-1/4 text-center">{{i.package_id.rate_id.description}}</td>
                          </tr>
  ...
  getAll(type,name){
    ...
    if(name){
      ...
      axios.get(process.env.VUE_APP_BASE_URL + '/user_sessions/get_visits/' + this.page + '/' + this.page_limit + '/' + type + '/' + name)
      .then((res)=>{
      ...
      axios.get(process.env.VUE_APP_BASE_URL + '/user_sessions/get_visits/' + this.page + '/' + this.page_limit + '/' + this.selected_date)
      .then((res)=>{
