<template>
  ...
  methods: {
    ...
    setItem(i,type){
      ...
      axios.post(process.env.VUE_APP_BASE_URL + '/user_types_menu/assign_item', this.select_data)
      .then((res)=>{
  ...
  getUserTypes(){
//
axios.get(process.env.VUE_APP_BASE_URL + '/user_types/all')
  .then((res)=>{
  
    if(res.status == 200){
  
      this.allusertypes = res.data[0];

      this.ut_id = this.allusertypes[0].id;
      this.getAll();
...
axios.get(process.env.VUE_APP_BASE_URL + '/user_menu/get_all')
  ...
  swapItem(id,action){
    ...
    if(action == 'remove'){
        axios.post(process.env.VUE_APP_BASE_URL + '/user_types_menu/delete', {id : id})
        .then((res)=>{
        ...
        axios.post(process.env.VUE_APP_BASE_URL + '/user_types_menu/add', data)
        .then((res)=>{
  ...
  axios.get(process.env.VUE_APP_BASE_URL + '/user_types_menu/get_menu/' + this.ut_id)
  .then((res)=>{
