...
@Controller('user_wallet_transaction')
export class UserWalletTransactionController {

    constructor(private i_service: UserWalletTransactionService) {
    ...
    async findByUser(@Param('id') id: number) {
        const response = await this.i_service.findByUser(id);
        return response;
    }
    ...
    @Post('add')
    async addItem(@Body() createUserOto: UserWalletTransactionEntity){
    ...
    @Post('update_wallet')
    async updateWallet(@Body() data: any){
        return await this.i_service.updateWallet(data);
    }
